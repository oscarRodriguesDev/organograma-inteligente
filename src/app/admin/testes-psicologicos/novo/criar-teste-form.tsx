...
import { criarTesteAction } from '@/lib/admin-psich-actions'
...
export function CriarTesteForm({ empresas }: { empresas: Empresa[] }) {
  ...
  const [tipo, setTipo] = useState('personalidade')
  const [empresasSelecionadas, setEmpresasSelecionadas] = useState<string[]>([])
  const [perguntas, setPerguntas] = useState<PerguntaForm[]>([
    { id: novaPerguntaId(), pergunta: '', tipo: 'escala_1_5', opcoes: [], peso: 1, ordem: 0, obrigatoria: true },
  ])
  ...
  function atualizarPergunta(id: string, campo: keyof PerguntaForm, valor: any) {
    setPerguntas(prev => prev.map(p => {
      if (p.id !== id) return p
      // Se mudou para escala_labeled e não tem opcoes, preenche padrão
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await criarTesteAction(formData)
      if (res.ok) {
        router.push('/admin/testes-psicologicos')
        router.refresh()
      } else {
        setErro(res.erro ?? 'Erro ao criar teste')
      }
    } catch (e) {
      ...
      <section className="bg-white dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6 space-y-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ex: Inventário de Personalidade"
            maxLength={200}
            required
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Breve descrição do objetivo do teste"
            rows={2}
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Ex: Responda cada pergunta com sinceridade. Não existem respostas certas ou erradas."
            rows={3}
            className="w-full px-3 py-2 text-sm border border-zinc-300 dark:border-zinc-700 rounded-lg bg-transparent text-foreground placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-400 resize-none"
          />
        </div>
