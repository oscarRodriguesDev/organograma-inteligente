...
import { removerInvestimentoAction } from '@/lib/admin-actions'
...
export function ExcluirInvestimentoButton({
  investimentoId,
  descricao,
}: {
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6 max-w-sm w-full mx-4 shadow-xl">
            <h3 className="text-lg font-semibold text-foreground mb-2">Excluir Investimento</h3>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
              Tem certeza que deseja excluir &ldquo;{descricao}&rdquo;? Esta ação não pode ser desfeita.
            </p>
            <div className="flex items-center justify-end gap-3">
              ...
              <form
                action={async () => {
                  await removerInvestimentoAction(investimentoId)
                  setOpen(false)
                }}
              >
                <SubmitButton />
