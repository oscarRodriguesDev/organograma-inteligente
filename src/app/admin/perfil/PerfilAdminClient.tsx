...
export default function PerfilAdminClient({ perfil }: { perfil: PerfilAdminData }) {
  ...
  const [fotoUrlAtual, setFotoUrlAtual] = useState<string | null>(perfil.fotoUrl)
  ...
  async function handleUploadFoto(file: File) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setFotoPreview(fotoUrlAtual)
        return
      }

      setFotoUrlAtual(data.fotoUrl)
    ...
    <>
        ...
        <form action={handlePerfil} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Seu username (opcional)"
              className="w-full px-3 py-2 text-sm border border-zinc-300 dark:border-zinc-700 rounded-lg bg-transparent text-foreground placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-400"
            />
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1">
              Opcional. Deixe em branco para manter o atual.
            </p>
          </div>
          ...
          <div className="text-xs text-zinc-400 dark:text-zinc-500">
            Email: <span className="text-foreground">{perfil.email ?? '—'}</span>
          </div>
      ...
      <div className="bg-white dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
          A única informação que exige confirmação para alteração é a senha.
        </p>
